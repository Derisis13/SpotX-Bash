...
BUILD_VERSION="1.2.13.661.ga588f749"
...
SXB_VERSION=$(echo ${BUILD_VERSION} | perl -ne '/(.*)\./ && print "$1"')
...
ver () { 
  echo "$@" | awk -F. '{ printf("%d%03d%04d%05d\n", $1,$2,$3,$4); }'
}
...
while getopts 'cdeF:fhioP:prUuv-:' flag; do
  case "${flag}" in
    -)
      case "${OPTARG}" in
        devmode) DEVELOPERMODE_OPT='true' ;;
...
SHELL="==gCk1mLF1ERBVkUvcXYy9SMidjZ0QzMkRTOwMWZlVTMmRTY2EmYlRGOmNGMzgDOl9yMylmZ0Vmav02bj5iY1hGdpdmL0NXan9yL6MHc0RHa"
...
if [[ "${PLATFORM_TYPE}" == "macOS" ]]; then
  ...
  STRING3="K0TPnNkNKdEZ1BzMU9kUFR1Q0hkS0BjbUBFbwU1UWtmVmJVRUpkVsF1NSNlZ5FlRVdjUTZWSOtWVCRHSKljREZVU0hkS"
...
if [[ "${PLATFORM_TYPE}" == "Linux" ]]; then
  if [ -z ${INSTALL_PATH+x} ]; then
    ...
    if [[ -d "${INSTALL_PATH}" && "${INSTALL_PATH}" != /usr*bin ]]; then
      ...
      INSTALL_PATH=$(timeout 20 find / -type f -path "*/spotify*Apps/*" -name "xpui.spa" -size -10M -size +3M -print -quit 2>/dev/null | rev | cut -d/ -f3- | rev)
      ...
      echo -e "\n${YELLOW}Warning:${CLEAR} Spotify PATH may be set to symlink, searching for Spotify directory...\n"
      INSTALL_PATH=$(timeout 20 find / -type f -path "*/spotify*Apps/*" -name "xpui.spa" -size -10M -size +3M -print -quit 2>/dev/null | rev | cut -d/ -f3- | rev)
      if [[ -d "${INSTALL_PATH}" && "${INSTALL_PATH}" != "/usr/bin" ]]; then
        echo -e "Found Spotify directory: ${INSTALL_PATH}\n"
...
if [[ "${PLATFORM_TYPE}" == "Linux" ]]; then
  ...
  CACHE_PATH=$(timeout 10 find / -type d -path "*cache/spotify*" -name "spotify" -print -quit 2>/dev/null)
...
if [[ "${PLATFORM_TYPE}" == "macOS" ]]; then
  if [[ "${NOTINSTALLED_VAR}" == 'false' ]]; then
    if [ -z ${FORCE_VERSION_FLAG+x} ]; then
      CLIENT_VERSION=$(perl -ne 'if($v=/ShortVersion/..undef){print if($v>1 && $v<=2)}' "${APP_PATH}/Contents/Info.plist" 2>/dev/null | perl -ne '/\>(.*)\./ && print "$1"') || VERSION_FAILED='true'
    fi
  fi
fi
if [[ "${PLATFORM_TYPE}" == "Linux" ]]; then
  if [ -z ${FORCE_VERSION_FLAG+x} ]; then
    "${APP_BINARY}" --version >/dev/null 2>/dev/null && CLIENT_VERSION=$("${APP_BINARY}" --version | cut -d " " -f3- | rev | cut -d. -f2- | rev) || VERSION_FAILED='true'
  fi
fi
...
AD_ESPERANTO='s|(this._product_state=(.))|$1,$2.putOverridesValues({pairs:{ads:'\''0'\'',catalogue:'\''premium'\'',product:'\''premium'\'',type:'\''premium'\''}})|'
...
AD_ENABLE_DESKTOP_MUSIC_LEAVE_BEHINDS='s|Enable music leavebehinds on eligible playlists for desktop",default:\K!.(?=})|false|s'

ENABLE_ADD_PLAYLIST='s|Enable support for adding a playlist to another playlist",default:\K!1|true|s'
ENABLE_ALBUM_COVER_ART_MODAL='s|Enable the cover art modal on the Album page",default:\K!1|true|s'
...
ENABLE_BETAMAX_SDK_SUBTITLES_DESKTOPX='s|Enables subtitles setting and rendering subtitles on the betamax SDK on DesktopX",default:\K!1|true|s'
...
ENABLE_CONCERTS_FOR_THIS_IS_PLAYLIST='s|Enable Tour Card on This is Playlist",default:\K!1|true|s'
...
ENABLE_MY_LITTLE_MERMAID_EASTER_EGG='s|Enable My Little Mermaid playlist easter egg",default:\K!1|true|s'
ENABLE_MY_LITTLE_MERMAID_EASTER_EGG_VIDEO='s|Enable My Little Mermaid playlist easter egg video background",default:\K!1|true|s'
...
ENABLE_PICK_AND_SHUFFLE='s|Enable pick and shuffle",default:\K!1|false|s'
...
ENABLE_SOCIAL_CONNECT_ON_DESKTOP='s|Enable the Social Connect API that powers group listening sessions for Desktop",default:\K!.(?=})|true|s'
...
ENABLE_WHATS_NEW_FEED='s|Enable the what.s new feed panel",default:\K!1|true|s'
...
HIDE_PODCASTS2='s#(!Array.isArray\(.\)\|\|.===..length\)return null;)#$1 for (let i=0; i<(e.children?e.children.length:e.length); i++) {const key=(e.children?.[i]||e[i])?.key; if(!key||key.match(/(episode|show)/)||(e.title||t)?.match(/podcasts/i)) return null;};#'
...
DEBUG_MENU='s/(return ).{1,3}(\?(.{1,4}createElement|\(.{1,7}.jsxs\))\(.{3,7}\{displayText:"Debug Tools")/$1true$2/'
DEV_LINUX_X64='s|\xFF\xFF\K\xE8..(?=\x00\x00\x89.\xF6\x85)|\xB8\x03\x00|'
DEV_MACOS_X64='s|\x95\x38\xFA\xFF\xFF\K\xE8..(?=\x00\x00\x89.\xF6)|\xB8\x03\x00|'
DEV_MACOS_ARM64='s|\x00\xB9\xE1.\x00\x91\xE2.\x00\x91\K..\x00\x94(?=\xF5\x03\x00\xAA\xE8)|\x60\x00\x80\xD2|'

BETAMAX_FILTER_NEGATIVE_DURATION='s|for duration that is negative",default:\K!.(?=})|false|s'
ENABLE_DSA='s|Enable showing DSA (Digital Services Act) context menu and modal for ads",default:\K!.(?=})|false|s'
...
ENABLE_USER_FRAUD_CANVAS='s|Enable user fraud Canvas Fingerprinting",default:\K!.(?=})|false|s'
ENABLE_USER_FRAUD_SIGNALS='s|Enable user fraud signals",default:\K!.(?=})|false|s'
...
if [[ "${INTERACTIVE_FLAG}" == "true" ]]; then
  ...
  if [[ "${PLATFORM_TYPE}" == "macOS" ]]; then
    if read_yn  "Block Spotify updates? " ; then
      UPDATE_FLAG='true';
    else
      UPDATE_FLAG='false';
    fi
...
if [[ ! -f "${XPUI_SPA}" ]]; then
  echo -e "${RED}Error:${CLEAR} Detected a modified Spotify installation!\nReinstall Spotify then try again. Exiting...\n"; exit
else
  if [[ ! -w "${XPUI_PATH}" ]]; then
...
if [[ "${XPUI_SKIP}" == "false" ]]; then
  ...
  [[ "${VERSION_FAILED}" == "true" ]] && CLIENT_VERSION=$(perl -ne '/clientVersion\:\"(.*)\",os\:.{1,20}\.getOSName/ && print "$1"' "${XPUI_JS}" | rev | cut -d'.' -f2- | rev)
  ...
  if [[ "${PREMIUM_FLAG}" == "false" ]]; then
    ...
    [[ $(ver "${CLIENT_VERSION}") -lt $(ver "1.1.93.896") ]] && $PERL "${AD_AUDIO_ADS}" "${XPUI_JS}"
    $PERL "${AD_BILLBOARD}" "${XPUI_JS}"
    $PERL "${AD_ENABLE_DESKTOP_MUSIC_LEAVE_BEHINDS}" "${XPUI_JS}"
    $PERL "${AD_EMPTY_AD_BLOCK}" "${XPUI_JS}"
    [[ $(ver "${CLIENT_VERSION}") -ge $(ver "1.1.93.896") ]] && $PERL "${AD_ESPERANTO}" "${XPUI_JS}"
    $PERL "${AD_LOGIC}" "${APP_BINARY}"
    $PERL "${AD_PLAYLIST_SPONSORS}" "${XPUI_JS}"
    $PERL "${AD_SLOT}" "${APP_BINARY}"
    [[ $(ver "${CLIENT_VERSION}") -lt $(ver "1.1.93.896") ]] && $PERL "${AD_UPGRADE_BUTTON}" "${XPUI_JS}"
    $PERL "${AD_UPSELL}" "${XPUI_JS}"
    $PERL "${BETAMAX_FILTER_NEGATIVE_DURATION}" "${XPUI_JS}"
    $PERL "${ENABLE_CONNECT_ESPERANTO_STATE}" "${XPUI_JS}"
    $PERL "${ENABLE_DSA}" "${XPUI_JS}"
  ...
  if [[ "${DEVELOPERMODE_OPT}" == "true" ]]; then
    [[ "${PLATFORM_TYPE}" == "Linux" ]] && $PERL "${DEV_LINUX_X64}" "${APP_BINARY}" && $PERL "${DEBUG_MENU}" "${XPUI_JS}"
    [[ "${PLATFORM_TYPE}" == "macOS" && "${ARCH}" == "x86_64" ]] && $PERL "${DEV_MACOS_X64}" "${APP_BINARY}" && $PERL "${DEBUG_MENU}" "${XPUI_JS}"
    [[ "${PLATFORM_TYPE}" == "macOS" && "${ARCH}" == "arm64" ]] && $PERL "${DEV_MACOS_ARM64}" "${APP_BINARY}" && $PERL "${DEBUG_MENU}" "${XPUI_JS}"
  ...
  if [[ "${EXCLUDE_EXPERIMENTAL_FLAG}" == "true" ]]; then
    ...
    $PERL "${ENABLE_ADD_PLAYLIST}" "${XPUI_JS}"
    $PERL "${ENABLE_ALBUM_COVER_ART_MODAL}" "${XPUI_JS}"
    $PERL "${ENABLE_ATTACK_ON_TITAN_EASTER_EGG}" "${XPUI_JS}"
    $PERL "${ENABLE_AUDIOBOOKS_CLIENTX}" "${XPUI_JS}"
    $PERL "${ENABLE_BAD_BUNNY}" "${XPUI_JS}"
    $PERL "${ENABLE_BETAMAX_SDK_SUBTITLES_DESKTOPX}" "${XPUI_JS}"
    $PERL "${ENABLE_BLACK_PANTHER}" "${XPUI_JS}"
    ...
    $PERL "${ENABLE_CONCERTS_FOR_THIS_IS_PLAYLIST}" "${XPUI_JS}"
    $PERL "${ENABLE_CONCERTS_INTERESTED}" "${XPUI_JS}"
    $PERL "${ENABLE_CULTURAL_MOMENTS}" "${XPUI_JS}"
    $PERL "${ENABLE_DEVICE_LIST_LOCAL}" "${XPUI_JS}"
    ...
    $PERL "${ENABLE_FOLLOWERS_ON_PROFILE}" "${XPUI_JS}"
    $PERL "${ENABLE_FORGET_DEVICES}" "${XPUI_JS}"
    $PERL "${ENABLE_HIFI}" "${XPUI_JS}"
    $PERL "${ENABLE_IGNORE_REC}" "${XPUI_JS}"
    $PERL "${ENABLE_INLINE_CURATION}" "${XPUI_JS}"
    $PERL "${ENABLE_JUST_PRESS_PLAY}" "${XPUI_JS}"
    $PERL "${ENABLE_LIKED_SONGS}" "${XPUI_JS}"
    $PERL "${ENABLE_LYRICS_CHECK}" "${XPUI_JS}"
    $PERL "${ENABLE_LYRICS_MATCH}" "${XPUI_JS}"
    $PERL "${ENABLE_LYRICS_NEW}" "${XPUI_JS}"
    $PERL "${ENABLE_MY_LITTLE_MERMAID_EASTER_EGG}" "${XPUI_JS}"
    $PERL "${ENABLE_MY_LITTLE_MERMAID_EASTER_EGG_VIDEO}" "${XPUI_JS}"
    [[ "${DISABLEMADEFORYOU_OPT}" == "false" ]] && $PERL "${ENABLE_MADE_FOR_YOU}" "${XPUI_JS}"
    $PERL "${ENABLE_NEW_EPISODES}" "${XPUI_JS}"
    $PERL "${ENABLE_NEXT_BEST_EPISODE}" "${XPUI_JS}"
    $PERL "${ENABLE_PATHFINDER_DATA}" "${XPUI_JS}"
    $PERL "${ENABLE_PICK_AND_SHUFFLE}" "${XPUI_JS}"
    $PERL "${ENABLE_PLAY_AT_FIRST_TAP}" "${XPUI_JS}"
    $PERL "${ENABLE_PLAYLIST_CREATION_FLOW}" "${XPUI_JS}"
    $PERL "${ENABLE_PLAYLIST_PERMISSIONS_FLOWS}" "${XPUI_JS}"
    $PERL "${ENABLE_PODCAST_PLAYBACK_SPEED}" "${XPUI_JS}"
    $PERL "${ENABLE_PODCAST_TRIMMING}" "${XPUI_JS}"
    $PERL "${ENABLE_PODCAST_TRANSCRIPTS}" "${XPUI_JS}"
    $PERL "${ENABLE_SEARCH_BOX}" "${XPUI_JS}"
    [[ "${DISABLESEARCHV3_OPT}" == "false" ]] && $PERL "${ENABLE_SEARCH_V3}" "${XPUI_JS}"
    $PERL "${ENABLE_SHOW_WRAPPED_BANNER}" "${XPUI_JS}"
    $PERL "${ENABLE_SIMILAR_PLAYLIST}" "${XPUI_JS}"
    $PERL "${ENABLE_SING_ALONG}" "${XPUI_JS}"
    [[ "${PREMIUM_FLAG}" == "true" ]] && $PERL "${ENABLE_SOCIAL_CONNECT_ON_DESKTOP}" "${XPUI_JS}"
    $PERL "${ENABLE_STRANGER_THINGS}" "${XPUI_JS}"
    $PERL "${ENABLE_SUBTITLES_AUTOGENERATED_LABEL}" "${XPUI_JS}"
    $PERL "${ENABLE_USER_PROFILE_EDIT}" "${XPUI_JS}"
    [[ $(ver "${CLIENT_VERSION}") -ge $(ver "1.2.12.902") ]] && $PERL "${ENABLE_WHATS_NEW_FEED}" "${XPUI_JS}"
    $PERL "${ENABLE_YLX_Typeahead_Search}" "${XPUI_JS}"
    [[ "${PREMIUM_FLAG}" == "true" ]] && $PERL "${ENABLE_YOUR_DJ}" "${XPUI_JS}"
...
if [[ "${CACHE_FLAG}" == "true" ]]; then
  rm -rf "${CACHE_PATH}/Browser" 2>/dev/null
  rm -rf "${CACHE_PATH}/Data" 2>/dev/null
  rm "${CACHE_PATH}/LocalPrefs.json" 2>/dev/null
  perl -e 'print "\xE2\x9C\x94\x20\x43\x6C\x65\x61\x72\x65\x64\x20\x61\x70\x70\x20\x63\x61\x63\x68\x65\n"'
fi

if [[ "${XPUI_SKIP}" == "false" ]]; then
  $PERL "${ENABLE_USER_FRAUD_CANVAS}" "${XPUI_JS}"
  $PERL "${ENABLE_USER_FRAUD_SIGNALS}" "${XPUI_JS}"
  $PERL "${ENABLE_USER_FRAUD_VERIFICATION}" "${XPUI_JS}"
  $PERL "${ENABLE_USER_FRAUD_VERIFICATION_REQUEST}" "${XPUI_JS}"
